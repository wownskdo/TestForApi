#!/usr/bin/env python3
"""
Script to automatically update ForSite Lua return files based on preset folder contents.
...
# Configuration for ForSite directory structure
PRESET_CATEGORIES = [
...
PRESETS_BASE_PATH = "ForSite/Presets"
RETURN_TABLES_PATH = "ForSite/returntables"
...
def update_return_file(category_config):
    ...
    print(f"Processing ForSite category: {category_name}")
    
    # Get files in the category
    files = get_files_in_category(category_name)
    print(f"Found {len(files)} files in ForSite/{category_name}")
    ...
    try:
        ...
        if os.path.exists(return_file_path):
            ...
            if existing_content == lua_content:
                print(f"No changes needed for ForSite {return_file}.lua")
                write_file = False
        
        if write_file:
            with open(return_file_path, 'w', encoding='utf-8') as f:
                f.write(lua_content)
            print(f"Updated ForSite {return_file}.lua with {len(files)} files")
            
    except Exception as e:
        print(f"Error updating ForSite {return_file}.lua: {e}")

def main():
    """Main function to update all ForSite return files."""
    print("Starting ForSite return file update process...")
    
    # Check if preset base path exists
    if not os.path.exists(PRESETS_BASE_PATH):
        print(f"Error: ForSite presets base path '{PRESETS_BASE_PATH}' does not exist")
        return
    ...
    print("ForSite return file update process completed!")
